...
movieNames = np.loadtxt(fname="./ml-100k/u.item",dtype={'names': ('id','name'), 'formats': (np.int,'S128')},delimiter='|' ,usecols=(0,1),) # replace 0 with the correct code to load the movie names
...
maxMovie = np.max(movieData['movie']+1) # replace 0 with the correct code
...
for rows in movieData:
    if rows['rating'] >= 4:
        userLikes[rows['user'],rows['movie']] = 1
